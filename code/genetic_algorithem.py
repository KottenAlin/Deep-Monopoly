...
class GeneticBot(Bot):
        ...
        for params in population:
            ...
            for param in mutated_params:
                ...
                if random.random() < mutation_rate:
                    ...
                    mutated_params[param] = max(0, min(1, mutated_params[param] + delta))
    ...
    def calculate_fitness(self, game_result):
        ...
        building_value = sum((p.houses * p.house_price) for p in self.player.properties 
                            if hasattr(p, 'houses') and p.houses > 0)
        building_value += sum((5 * p.house_price) for p in self.player.properties 
    ...
    @staticmethod
    def run_genetic_algorithm(generations=20, population_size=30, games_per_individual=5):
        ...
        for gen in range(generations):
            ...
            for i, params in enumerate(population):
                ...
                for _ in range(games_per_individual):
                    # Set up a new game
                    game = MonopolyGame(bot_count=4, neural_bot_count=0, bots_parameters=bots_parameters)
                ...
                print(f"Individual {i+1}/{len(population)}: Fitness = {avg_fitness:.2f}")
            ...
            print(f"Generation {gen+1} complete. Best fitness: {max(fitness_scores):.2f}, Avg fitness: {sum(fitness_scores)/len(fitness_scores):.2f}")
    ...
    @staticmethod
    def train(generations=20, population_size=30, games_per_individual=5, save_results=True):
    ...
    def make_move(self):
        ...
        old_buildings = sum(getattr(p, 'houses', 0) for p in self.player.properties) + \
        ...
        new_buildings = sum(getattr(p, 'houses', 0) for p in self.player.properties) + \
                        sum(1 for p in self.player.properties if getattr(p, 'hotel', False))
...
def main():
    ...
    game = MonopolyGame(bot_count=4, neural_bot_count=0, bots_parameters=best_parameters)
