...
class NeuralAlgorithm:
    def __init__(self, learning_rate=0.001, memory_size=1000, batch_size=32):
    ...
    def create_game_state_vector(self, game, player=None):
        ...
        if player:
            ...
            # Property details
            mortgaged_count = sum(1 for p in player.properties if hasattr(p, 'status') and p.status == PropertyStatus.MORTGAGED)
            state.append(mortgaged_count / max(1, len(player.properties)))
            
            # Development level
            total_houses = sum(getattr(p, 'houses', 0) for p in player.properties)
            total_hotels = sum(1 for p in player.properties if getattr(p, 'hotel', False))
        ...
        if game:
            ...
            available_properties = sum(1 for s in game.board.spaces 
                                      if hasattr(s, 'owner') and s.owner is None)
        ...
        if game and len(game.players) > 1:
            ...
            if opponents:
                ...
                avg_development = sum(sum(getattr(p, 'houses', 0) for p in opp.properties) 
                                     for opp in opponents) / max(1, len(opponents))
        ...
        for i in range(40):
            ...
            if space and hasattr(space, 'owner'):
                ...
                is_mortgaged = hasattr(space, 'status') and space.status == PropertyStatus.MORTGAGED
    ...
    def calculate_reward(self, game, player, starting_win_prob, ending_win_prob):
        ...
        # Combine all factors
        return reward + money_factor + property_factor + development_factor + monopoly_factor
    ...
    def train_from_memory(self):
        ...
        # Prepare batch data
        states = torch.stack([self._ensure_state_size(experience[0]) for experience in batch])
        parameters = torch.tensor([experience[1] for experience in batch], dtype=torch.float32)
        rewards = torch.tensor([experience[2] for experience in batch], dtype=torch.float32).view(-1, 1)
        next_states = torch.stack([self._ensure_state_size(experience[3]) for experience in batch])
        ...
        for i in range(len(batch)):
            ...
            if reward_factor > 0:
                ...
                target[i] = torch.clamp(predicted[i] + adjustment * (0.5 - parameters[i]), 0, 1)
    ...
    def run_training_games(self, num_games=20, num_epochs=50, display_progress=True):
        ...
        for epoch in range(num_epochs):
            ...
            for game_num in range(num_games):
                ...
                # Calculate reward
                reward = self.calculate_reward(game, neural_bot_player, initial_win_prob, final_win_prob)
                total_reward += reward
            ...
            print(f"  Win rate: {win_rate:.2f}, Avg reward: {avg_reward:.4f}, Avg turns: {avg_turns:.1f}")
        ...
        return {
            ...
            "final_parameters": self.get_bot_parameters(torch.zeros(self.input_size, dtype=torch.float32))
        }
    ...
    def save_model(self, filepath="models/neural_algorithm_model.pth"):
        ...
        params = self.get_bot_parameters(torch.zeros(self.input_size, dtype=torch.float32))
...
class ActionNeuralBot(Bot):
    ...
    def new_initialise(self, model):
        ...
        needs_adapter = (input_size != self.input_dim) or (output_size != self.output_dim)
        ...
        if needs_adapter:
            ...
            if input_size != self.input_dim:
                self.input_adapter = nn.Sequential(
                    nn.Linear(self.input_dim, input_size),
                    nn.ReLU()
                )
            else:
            ...
            if output_size != self.output_dim:
                ...
                for layer in self.output_adapter:
                    if isinstance(layer, nn.Linear):
                        ...
                        layer.bias.data.fill_(0.0)  # Initialize to produce ~0.5 outputs after sigmoid
            else:
            ...
            self._get_action_probabilities = types.MethodType(adapted_get_action_probabilities, self)
        else:
    ...
    def _get_state(self):
        ...
        # Property details
        mortgaged_count = sum(1 for p in self.player.properties if hasattr(p, 'status') and p.status == PropertyStatus.MORTGAGED)
        state.append(mortgaged_count / max(1, len(self.player.properties)))
        
        # Development level
        total_houses = sum(getattr(p, 'houses', 0) for p in self.player.properties if hasattr(p, 'houses'))
        total_hotels = sum(1 for p in self.player.properties if hasattr(p, 'hotel') and p.hotel)
        state.append(total_houses / 32.0)  # Max 32 houses
        state.append(total_hotels / 12.0)  # Max 12 hotels
        ...
        # Property availability
        available_properties = sum(1 for s in self.game.board.spaces 
                                   if hasattr(s, 'owner') and s.owner is None)
        state.append(available_properties / 28.0)
        
        # Opponent information
        opponents = [p for p in self.game.players if p != self.player and not p.bankrupt]
        if opponents:
            avg_money = sum(p.money for p in opponents) / len(opponents)
            state.append(avg_money / 2000.0)
            ...
            for opp in opponents:
                houses = sum(getattr(p, 'houses', 0) for p in opp.properties if hasattr(p, 'houses'))
                hotels = sum(1 for p in opp.properties if hasattr(p, 'hotel') and p.hotel)
        ...
        for i in range(40):
            if i < len(self.game.board.spaces):
                ...
                if hasattr(space, 'owner'):
                    ...
                    mortgaged = hasattr(space, 'status') and space.status == PropertyStatus.MORTGAGED
                    state.append(1.0 if space.owner == self.player and mortgaged else 0.0)
                    
                    # Development: houses/5 if player-owned, 0 otherwise
                    houses = getattr(space, 'houses', 0) if space.owner == self.player else 0
    ...
    def decide_mortgage_property(self, amount_needed):
        ...
        mortgage_decision = action_probs[6].item()  # Seventh output is mortgage_property
    ...
    def decide_unmortgage_property(self):
        ...
        unmortgage_decision = action_probs[7].item()  # Eighth output is unmortgage_property
    ...
    def make_move(self):
        ...
        if len(action_probs) < 7:
            print(f"Warning: Action probabilities has only {len(action_probs)} values, expected at least 7")
            ...
            trade_initiation_prob = action_probs[min(5, len(action_probs)-1)].item() if len(action_probs) > 5 else 0.3
            unmortgage_prob = action_probs[min(7, len(action_probs)-1)].item() if len(action_probs) > 7 else 0.4
            mortgage_prob = action_probs[min(6, len(action_probs)-1)].item() if len(action_probs) > 6 else 0.3
    ...
    def _calculate_reward(self, initial_state, final_state, initial_money):
        ...
        max_idx = min(self.input_dim - 3, 120)  # Ensure we don't go beyond tensor bounds
        ...
        initial_houses = sum(initial_state[i+2].item() for i in range(14, max_idx, 3) if i+2 < self.input_dim)
        final_houses = sum(final_state[i+2].item() for i in range(14, max_idx, 3) if i+2 < self.input_dim)
        ...
        initial_mortgaged = sum(initial_state[i+1].item() for i in range(14, max_idx, 3) if i+1 < self.input_dim)
        final_mortgaged = sum(final_state[i+1].item() for i in range(14, max_idx, 3) if i+1 < self.input_dim)
        if final_mortgaged > initial_mortgaged:
            reward -= 0.1 * (final_mortgaged - initial_mortgaged)  # Small penalty for mortgaging
        
        # Reward for unmortgaging
        if final_mortgaged < initial_mortgaged:
            reward += 0.15 * (initial_mortgaged - final_mortgaged)  # Reward for unmortgaging
...
def main():
    ...
    # Run training games
    results = algorithm.run_training_games(num_games=20, num_epochs=5)
